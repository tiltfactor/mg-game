...
class Hybrid_Provider_Adapter
{
	...
	*/
	function login()
	{
		...
		$this->params["login_done"]  = 'http://' /*.$_SERVER['SERVER_NAME']*/ .'localhost' . $HYBRID_AUTH_URL_BASE . ( strpos( $HYBRID_AUTH_URL_BASE, '?' ) ? '&' : '?' ) . "hauth.done={$this->id}";
    ...
    //    print_r( $this->params["login_start"]); //pkostov test*/
     //   die(); //pkostov test*/
