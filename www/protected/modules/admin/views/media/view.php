...
if($media_type === 'image') {
    $media = CHtml::link(
        CHtml::image($model->institution->url . '/www'. UPLOAD_PATH . '/thumbs/'. $model->name) . ' [' . Yii::t('app', 'zoom') . ']',
        $model->institution->url . '/www'. UPLOAD_PATH . '/images/'. $model->name,
        array('rel'=>'zoom', 'media_type'=> $media_type, 'class'=>'zoom'));
} else if($media_type === 'video') {
    $url_webm = $model->institution->url . '/www'. UPLOAD_PATH . '/videos/'. urlencode($model->name);
    $url_mp4 = $model->institution->url . '/www'. UPLOAD_PATH . '/videos/'. urlencode(substr($model->name, 0, -4)."mp4");
    $url_poster = $model->institution->url . '/www' . UPLOAD_PATH . '/videos/'. urlencode(substr($model->name, 0, -4)."jpeg");
    $media = '<video class="video" controls preload poster="'.$url_poster.'">
            <source src="'.$url_mp4.'"></source>
            <source src="'.$url_webm.'"></source>
        </video>';
} else {
    $url_mp3 = $model->institution->url . '/www' . UPLOAD_PATH . '/audios/'. urlencode($model->name);
    $url_ogg = $model->institution->url . '/www' . UPLOAD_PATH . '/audios/'. urlencode(substr($model->name, 0, -3)."ogg");
