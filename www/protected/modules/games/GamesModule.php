...
class GamesModule extends CWebModule
{
    ...
    public static function loadGame($unique_id, $active = true, $game_id = null)
    {
        ...
        if ($registered_game || !$active) {
            ...
            $game->nlp_api_base_url = Yii::app()->fbvStorage->get('nlpApiUrl');
     ...
     * @return MGMultiPlayer|null
     * @throws CHttpException
     */
    public static function getMultiplayerEngine($unique_id)
    {
        $game_engine = null;
        try {
            Yii::import("games.components.*");
    ...
    {
        $staticModel = GxActiveRecord::model($unique_id);
        $attr = array("unique_id" => $unique_id);
        if ($active)
            $attr["active"] = 1;
        return $staticModel->findByAttributes($attr);
    }
