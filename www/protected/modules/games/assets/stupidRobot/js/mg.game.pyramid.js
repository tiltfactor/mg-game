MG_GAME_STUPIDROBOT = function ($) {
    return $.extend(MG_GAME_API, {
        ...
        init: function (options) {
            ...
            $('#countdown').countdown({until: '+2m', layout: '{mnn}{sep}{snn}', onExpiry: MG_GAME_STUPIDROBOT.liftOff});
            ...
            $("#new_image").click(function (event) {
                ...
                $('#countdown').countdown({until: '+1s', layout: '{mnn}{sep}{snn}', onExpiry: MG_GAME_STUPIDROBOT.liftOff});
                //location.reload();
            });
            ...
            $("#pass").click(function () {
                ...
                new_period+= periods[6] - MG_GAME_STUPIDROBOT.pass_penalty + 's';

                if (MG_GAME_STUPIDROBOT.pass_penalty > 0 && (periods[5]*60 + periods[6]) < (MG_GAME_STUPIDROBOT.pass_penalty + 5)) {
                    // $("#pass").remove();
                    // you can not pass a level its too late!
                } else {
                    $('#countdown').countdown('destroy');

                    $('#countdown').countdown({until: new_period, layout: '{mnn}{sep}{snn}', onExpiry: MG_GAME_STUPIDROBOT.liftOff});
                    ...
                    var accepted = {
                        level: (MG_GAME_STUPIDROBOT.level + 1),
                        tag: ""
                    };

                    MG_GAME_STUPIDROBOT.levels.push(accepted);
                    MG_GAME_STUPIDROBOT.nextlevel(true);
            ...
            MG_GAME_STUPIDROBOT.wordField = $("#word");

            // submit on enter
            MG_GAME_STUPIDROBOT.wordField.focus().keydown(function (event) {
                if (event.keyCode == 13) {
            ...
            MG_GAME_STUPIDROBOT.submitButton = $("#button-play").click(MG_GAME_STUPIDROBOT.onsubmit);
        ...
        renderTurn: function (response) {
            ...
            if(MG_GAME_STUPIDROBOT.level <= 1) {
//                $("#stage").hide();
                ...
                $("#licences").html("");
                $("#template-licence").tmpl(MG_GAME_STUPIDROBOT.licence_info).appendTo($("#licences"));

                $("#more_info").html("");

                if (MG_GAME_STUPIDROBOT.more_info != null &&
                    MG_GAME_STUPIDROBOT.more_info.hasOwnProperty("url"))
                    $("#template-more-info").tmpl(MG_GAME_STUPIDROBOT.more_info).appendTo($("#more_info"));

                $("a[rel='zoom']").fancybox({overlayColor:'#000'});

                $("#stage").fadeIn(1000, function () {
                    MG_GAME_STUPIDROBOT.busy = false;
        ...
        renderFinal:function () {
            var matched_words = 0;
            for (var i in MG_GAME_STUPIDROBOT.levels) {
                var level = MG_GAME_STUPIDROBOT.levels[i];
            ...
            //finalMsg:"You reached " + (MG_GAME_STUPIDROBOT.level+MG_GAME_STUPIDROBOT.level_step -1) + " letters! How far can you go?"
            var final_info = {
            ...
            if (MG_GAME_STUPIDROBOT.more_info != null && MG_GAME_STUPIDROBOT.more_info.hasOwnProperty("url"))
                $("#template-more-info").tmpl(MG_GAME_STUPIDROBOT.more_info).appendTo($("#more_info"));


            $("#image_container").html("");
            if (MG_GAME_STUPIDROBOT.game.play_once_and_move_on == 1) {
                var info = {
                    remainingTime:null,
                ...
                }, MG_GAME_STUPIDROBOT.playOnceMoveOnFinalScreenWaitingTime);

                var updateRemainingTime = function () {
                    MG_GAME_STUPIDROBOT.playOnceMoveOnFinalScreenWaitingTime -= 1000;
                    if (MG_GAME_STUPIDROBOT.playOnceMoveOnFinalScreenWaitingTime >= 1) {
                        $('#remainingTime').text(MG_GAME_STUPIDROBOT.playOnceMoveOnFinalScreenWaitingTime / 1000);
...
*/
            ...
            console.log("about to renderTurn");
        ...
        onsubmit:function () {
            if (!MG_GAME_STUPIDROBOT.busy) {
                ...
                if (tags == "") {
                    ...
                    $().toastmessage("showToast", {
                        text: "not enough letters!",//"That wasn't a " + (MG_GAME_STUPIDROBOT.level + MG_GAME_STUPIDROBOT.level_step) + " letters word!",
                        position:"tops-center",
                        type:"notice",
                        background: "#F1F1F1"
                    });
        ...
        nextlevel:function (skip) {
            ...
            MG_GAME_STUPIDROBOT.wordField.attr("placeholder", "Enter a " + (MG_GAME_STUPIDROBOT.level + MG_GAME_STUPIDROBOT.level_step) + " letter word");
            $("#content").find("footer").removeClass("footer_level_" + MG_GAME_STUPIDROBOT.level -1).addClass("footer_level_" + MG_GAME_STUPIDROBOT.level); 		
            //$("#content").find("footer").removeClass("level_" + MG_GAME_STUPIDROBOT.level -1).addClass("level_" + MG_GAME_STUPIDROBOT.level);
            $("input#word").removeClass("level_" + MG_GAME_STUPIDROBOT.level -1).addClass("level_" + MG_GAME_STUPIDROBOT.level);
// Comment out. Pass button should be same color, regardless of level
//             $("#pass").removeClass("level_" + MG_GAME_STUPIDROBOT.level -1).addClass("level_" + MG_GAME_STUPIDROBOT.level);
