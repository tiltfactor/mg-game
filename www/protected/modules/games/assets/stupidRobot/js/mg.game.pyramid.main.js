MG_GAME_STUPIDROBOT = function ($) {
    return $.extend(MG_GAME_API, { // TODO make the extension work or remove
        ...
        //api_url : MG_STUPIDROBOT.api_url,
...
var actions = function (action, click_parent) {
   ...
   // MG_API.settings.api_url = MG_STUPIDROBOT.api_url; // <--------------------------------------------we use it here
    ...
    switch (action) {
        ...
        case 'game_customize':
            ...
            $('input#new_interest').unbind("keypress").keypress(function (e) {
                if (e.which == 13) {
                    ...
                    for (var i = 0; i < array.length; i++) {
                        MG_API.ajaxCall('/multiplayer/addInterest/gid/' + MG_STUPIDROBOT.gid + '/interest/' + encodeURIComponent($.trim(array[i])), function (institution_response) {
                            counter_i++;
                            if (i === counter_i) {
                                $('#new_interest').attr('value', '');
            ...
            MG_API.ajaxCall('/multiplayer/getInstitutions/gid/' + MG_STUPIDROBOT.gid, function (institution_response) {
                var json = {};
                ...
                $("#template-favorite_institutions").tmpl(json).appendTo($("#game_customize")).after(function () {
                    ...
                    $("#list_institutions .institution").off('click').on('click', function (e) {
                        e.preventDefault();
                        var row = $(this);
                        MG_STUPIDROBOT.institution_id = row.attr('institution_id');
                        MG_STUPIDROBOT.back_location = 'game_customize';
                        // institution_info
            ...
            $('#register #btn_register').off('click').on('click', function (e) {
                ...
                if ($("#register #username").val().length < 6 && $("#register #password").val().length < 6 && $("#register #verifyPassword").val() < 6 && $("#register #email").val().length < 5) {
                    $().toastmessage("showToast", {
                        ...
                        background:"white",
                        color:"black",
                        stayTime:MG_GAME_STUPIDROBOT.toastStayTime,
                        addClass:MG_GAME_STUPIDROBOT.toastBackgroundClass
                    });
                } else {
                    MG_API.ajaxCall('/user/register', function (response) { // we dont have that action
                        if (response.status === 'ok') {
                            MG_STUPIDROBOT.isLogged = true;
                            MG_STUPIDROBOT.username = $("#register #username").val();
                            MG_STUPIDROBOT.email = $("#register #email").val();
        ...
        case 'logout':
            MG_API.ajaxCall('/user/logout', function () {
                MG_STUPIDROBOT.isLogged = false;
                MG_STUPIDROBOT.username = "";
                MG_STUPIDROBOT.email = "";
                MG_API.ajaxCall('/user/sharedsecret', function (response) {
                    if (MG_API.checkResponse(response)) {
                        if (response.shared_secret !== undefined && response.shared_secret !== "") {
                            MG_API.settings.shared_secret = response.shared_secret;
        ...
        case 'account':
            ...
            //getBookmarks
            MG_API.ajaxCall('/multiplayer/getBookmarks/gid/' + MG_STUPIDROBOT.gid, function (account_bookmarks) {
                var json = {};
            ...
            // Interests
            MG_API.ajaxCall('/multiplayer/getInterests/gid/' + MG_STUPIDROBOT.gid, function (account_interest) {
                var json = {};
                    ...
                    $("#account_interest .delete").each(function () {
                        $(this).off('click').on('click', function (e) {
                            ...
                            confirmPretty("Do you really want to remove the interest.", function () {
                                MG_API.ajaxCall('/multiplayer/removeInterest/gid/' + MG_STUPIDROBOT.gid + '/id/' + row_id + '/', function (response) {
                                    row.remove();
                                });
                            })
                        });
            ...
            MG_API.ajaxCall('/multiplayer/getInstitutions/gid/' + MG_STUPIDROBOT.gid, function (account_playlist) {
                var json = {};
                ...
                $("#template-account_playlist").tmpl(json).appendTo($("#account_playlist")).after(function () {
                    ...
                    $("#account_playlist .row .institution").each(function (event) {
                        $(this).off('click').on('click', function (e) {
                            ...
                            var row = $(this).closest(".row");
                            MG_STUPIDROBOT.institution_id = row.attr('institution_id');
                            MG_STUPIDROBOT.back_location = 'account';
                            // institution_info
                    ...
                    $("#account_playlist .delete").each(function () {
                        $(this).off('click').on('click', function (e) {
                            ...
                            confirmPretty("Do you really want to disable medias from the institution.", function () {
                                MG_API.ajaxCall('/multiplayer/banInstitution/gid/' + MG_STUPIDROBOT.gid + '/id/' + row_id + '/', function (response) {
                                    row.remove();
                                });
                            })
                        });
        ...
        case 'account_update':
            ...
            userSettings.username = MG_STUPIDROBOT.username;
            userSettings.email = MG_STUPIDROBOT.email;
            $("#template-account_update").tmpl(userSettings).appendTo($("#account_update")).after(function () {
                ...
                $("#header").find('.back').off('click').on('click', function (e) {
                    ...
                    MG_STUPIDROBOT.back_location = null;
                    actions('account', '');
                    return false;
                });
                ...
                $('#account_update #btn_update').off('click').on('click', function (e) {
                    ...
                    MG_API.ajaxCall('/user/update', function (response) {
                        ...
                        $().toastmessage("showToast", {
                            ...
                            background:"white",
                            color:"black",
                            stayTime:MG_STUPIDROBOT.toastStayTime,
                            addClass:MG_STUPIDROBOT.toastBackgroundClass
                        });
                    }, {
                        type:'post',
                        data:{
        ...
        case 'institution_info':
           ...
           // $("#header").find('.back').show(); // <-- TODO this dont work and we can not see the upper left back arrow
            ...
            MG_API.ajaxCall('/multiplayer/GetInstitution/gid/' + MG_STUPIDROBOT.gid + '/id/' + MG_STUPIDROBOT.institution_id + '/', function (response) {
                ...
                function addClickFav() {
                    $("#header").find('.favorite').off('click').on('click', function () { // this is the upper-right "Remove from  Playlist"
                        ...
                        if ($(this).attr('type') === 'remove') {
                            MG_API.ajaxCall('/multiplayer/banInstitution/gid/' + MG_STUPIDROBOT.gid + '/id/' + MG_STUPIDROBOT.institution_id + '/', function (response) {
                                setTimeout(function () {
                                }, 3000);
                                $("#header").append(inst_add);
                                addClickFav();
                            ...
                            MG_API.ajaxCall('/multiplayer/unbanInstitution/gid/' + MG_STUPIDROBOT.gid + '/id/' + MG_STUPIDROBOT.institution_id + '/', function (response) {
                                setTimeout(function () {
                                }, 3000);
                                $("#header").append(inst_remove);
                                addClickFav();
                ...
                $("#header").find('.back').off('click').on('click', function (e) {
                    ...
                    $("#header").find('.favorite').remove();
                    $("#header").find('.setting').show();
                    click_parent = MG_STUPIDROBOT.back_location;
                    $("a[location='" + MG_STUPIDROBOT.back_location + "']").click();
                    return false;
                });
...
var setAuthentication = function () {
    $("#btn_login").off('click').on('click', function (e) {
        ...
        if ((jQuery.trim($("#login #username").val()).length + jQuery.trim($("#login #password").val()).length) < 1) {
            ...
            MG_API.ajaxCall('/user/login', function (response) {
                    if (response.status === 'ok') {
                        MG_STUPIDROBOT.isLogged = true;
                        MG_STUPIDROBOT.username = jQuery.trim($("#login #username").val());
    ...
    $("#facebook").off('click').on('click', function () {
        window.location.href = MG_STUPIDROBOT.arcade_url + "/site/login/provider/facebook?backUrl=" + encodeURIComponent(MG_STUPIDROBOT.game_base_url + '/' + MG_STUPIDROBOT.gid);
    });
}
...
var isLoggedUser = function() {
    if(MG_STUPIDROBOT.isLogged == 'true'){
        MG_API.ajaxCall('/user/sharedsecret', function (response) {
            if(response.status === 'ok') {
                MG_API.settings.shared_secret = response.shared_secret;
            }
...
$(document).ready(function() {
	return;
    MG_API.settings.api_url = MG_STUPIDROBOT.api_url;
